...
async function example3_quickTest() {
  ...
  try {
    ...
    console.log(`   - Environment: ${report.environment.strapiBaseUrl ? "Configured" : "Not configured"}`);
...
async function example4_customConfigTest(csvPath: string) {
  ...
  try {
    ...
    console.log(`   - Time: ${(sequentialResult.processingTime / 1000).toFixed(2)}s`);
    ...
    console.log(`   - Time: ${(parallelResult.processingTime / 1000).toFixed(2)}s`);
    
    // Compare performance
    const speedup = sequentialResult.processingTime / parallelResult.processingTime;
    console.log(`\n⚡ Performance Comparison:`);
    console.log(`   - Sequential: ${(sequentialResult.processingTime / 1000).toFixed(2)}s`);
    console.log(`   - Parallel: ${(parallelResult.processingTime / 1000).toFixed(2)}s`);
...
async function example5_advancedTestRunner(csvPath: string) {
  ...
  try {
    ...
    console.log(`   - Average per Record: ${(result.processingTime / result.totalRecords).toFixed(0)}ms`);
...
async function runExamples() {
  ...
  console.log("This script demonstrates various ways to test the migration lambda locally.\n");
  ...
  try {
    ...
    console.log("   4. Integrate the test runner into your development workflow");
